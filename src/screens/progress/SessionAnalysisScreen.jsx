...
export default function SessionAnalysisScreen({ go, state, set }) {
  const topicTitle = state?.lastQuizTopic || 'Fractions';

  const score = state?.lastQuizScore ?? 4;
  const total = state?.lastQuizTotal ?? 5;
  const pct = Math.round((score / total) * 100);
  const mistakes = state?.lastQuizMistakes || [];

  // LLM quiz feedback: { win, pattern, next_step }
  const [feedback, setFeedback] = useState(null);   // null = loading
  ...
  return (
    <div style={{ minHeight: '100%', background: 'var(--bg)', position: 'relative' }}>
      <VTopBar transparent showBack onBack={() => go('home')} title="Analysis" />
      <div style={{ padding: '72px 22px 100px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div className="v-enter">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 20px', background: scoreBg, borderRadius: 18, border: '1px solid ' + scoreColor + '33' }}>
            <div style={{ textAlign: 'center', flexShrink: 0 }}>
              <div style={{ fontFamily: "'Quicksand','Nunito',system-ui,sans-serif", fontSize: 44, fontWeight: 800, color: scoreColor, lineHeight: 1 }}>{score}</div>
              <div style={{ fontFamily: 'Inter', fontSize: 11, color: scoreColor + 'aa', marginTop: 2 }}>out of {total}</div>
            </div>
            <div style={{ width: 1, height: 48, background: scoreColor + '33', flexShrink: 0 }} />
            <div>
              <div style={{ fontFamily: "'Quicksand','Nunito',system-ui,sans-serif", fontSize: 16, fontWeight: 700, color: scoreColor, marginBottom: 3 }}>
                {pct >= 80 ? 'Strong performance' : pct >= 60 ? 'Good progress' : 'Needs more practice'}
              </div>
              <div style={{ fontFamily: 'Inter', fontSize: 12, color: 'var(--muted)', lineHeight: 1.5 }}>
                {pct >= 80 ? 'You have a solid grasp of this topic.' : pct >= 60 ? 'A few gaps to close — see below.' : 'Focus on the improvement areas below.'}
              </div>
            </div>
          </div>
        </div>
        ...
        {!feedback && (
          <div className="v-enter" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '18px 20px', background: '#fff', borderRadius: 14, border: '1px solid var(--border)' }}>
            ...
            <div style={{ fontFamily: 'Inter', fontSize: 13, color: 'var(--muted)' }}>Vidya is analysing your answers…</div>
          </div>
        )}
