...
const QUIZ_REVIEW_BANK: ReviewItem[] = [
  {
...
export default function QuizReviewQuestionScreen({ go, state, set }: ScreenProps) {
  const { t } = useTranslation('quiz');
  const total = QUIZ_REVIEW_BANK.length;
  const startIdx = Math.min(Math.max(0, (state?.reviewIdx as number) || 0), total - 1);
  const [idx, setIdx] = useState(startIdx);
  const Q = QUIZ_REVIEW_BANK[idx];
  const back = (state?.reviewFrom as ScreenId) || 'quiz-result-summary';
    ...
    <div style={{ minHeight: '100%', background: 'var(--bg)', position: 'relative' }}>
      ...
      <div style={{ padding: '72px 24px 32px', display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <span className="v-eyebrow">{t('reviewQuestion.review')}</span>
          <span style={{ width: 3, height: 3, borderRadius: 9999, background: 'var(--muted-2)' }} />
          <span style={{ fontSize: 11, letterSpacing: '0.05em', color: 'var(--muted-2)', textTransform: 'uppercase' }}>{Q.topic} · {t('reviewQuestion.marks', { count: Q.marks })}</span>
        </div>
        ...
        <div className="v-enter-fade" key={'pill-' + idx} style={{
          ...
          {Q.ok ? t('reviewQuestion.gotRight') : t('reviewQuestion.notQuite')}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
          <div className="v-card-soft" style={{
            ...
            <div className="v-eyebrow-sm" style={{ marginBottom: 6, color: 'var(--muted-2)' }}>{t('reviewQuestion.yourAnswer')}</div>
            <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 22, color: Q.ok ? 'var(--accent-success)' : 'var(--accent-warn)' }}>{Q.your}</div>
          </div>
          <div className="v-card-soft" style={{ padding: 14 }}>
            <div className="v-eyebrow-sm" style={{ marginBottom: 6, color: 'var(--muted-2)' }}>{t('reviewQuestion.correctAnswer')}</div>
            <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 22, color: 'var(--ink)' }}>{Q.correct}</div>
          </div>
        </div>

        {!Q.ok && Q.misread && (
          <div className="v-card" style={{ padding: '14px 16px', marginBottom: 14, background: '#FFFBEB', borderColor: 'rgba(245,158,11,0.25)' }}>
            <div className="v-eyebrow-sm" style={{ color: '#8B4513', marginBottom: 4 }}>{t('reviewQuestion.whereWrong')}</div>
            <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.45 }}>{Q.misread}</div>
          </div>
        )}

        <div className="v-card" style={{ padding: 18, marginBottom: 14 }}>
          <div className="v-eyebrow-sm" style={{ marginBottom: 8 }}>{t('reviewQuestion.why')}</div>
          <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.55, marginBottom: 14 }}>{Q.explain}</div>
          <div className="v-eyebrow-sm" style={{ marginBottom: 10 }}>{t('reviewQuestion.working')}</div>
          <ol style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {Q.steps.map((s, i) => (
              <li key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <span style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 15, color: 'var(--ink-2)', lineHeight: 1.45 }}>{s}</span>
              </li>
            ))}
          </ol>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 22 }}>
          <button className="v-btn-secondary v-tap" onClick={() => go('learn-concept')} style={{ flex: 1 }}>{t('reviewQuestion.openConcept')}</button>
          <button className="v-btn-secondary v-tap" onClick={() => go('practice')} style={{ flex: 1 }}>{t('reviewQuestion.practiceSimilar')}</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="v-btn-secondary v-tap" onClick={goPrev} disabled={idx === 0} style={{ flex: 1, opacity: idx === 0 ? 0.45 : 1 }}>
            <VIcon name="arrow-left" size={14} /> {t('reviewQuestion.prev')}
          </button>
          {idx < total - 1 ? (
            <button className="v-btn-primary v-tap" onClick={goNext} style={{ flex: 1 }}>
              {t('reviewQuestion.next')} <VIcon name="arrow-right" size={14} color="#fff" />
            </button>
          ) : (
            <button className="v-btn-primary v-tap" onClick={() => { set && set({ reviewIdx: 0 }); go(back); }} style={{ flex: 1 }}>
              {t('reviewQuestion.done')} <VIcon name="check" size={14} color="#fff" />
            </button>
          )}
        </div>
      </div>
