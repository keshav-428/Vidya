...
import type { ScreenProps, GradeResult } from '../../types';
...
interface ExamGradeResult extends GradeResult {
  overall_feedback?: string;
  improvement_areas?: string[];
}

export default function ExamEvalResultsScreen({ go, state }: ScreenProps) {
  const { t } = useTranslation('exam');
  const result = state?.examResult as ExamGradeResult | 'error' | null | undefined;
      ...
      <div style={{ minHeight: '100%', background: 'var(--bg)' }}>
        ...
        <div style={{ padding: '72px 24px 32px', textAlign: 'center' }}>
          <div style={{ fontFamily: 'Inter', fontSize: 14, color: 'var(--muted)', marginBottom: 20 }}>
            {result === 'error' ? t('evalResults.errorEvaluate') : t('evalResults.noEvaluation')}
          </div>
          <button className="v-btn-primary v-tap" onClick={() => go('exam-photo')}>{t('evalResults.reuploadSheets')}</button>
          <button className="v-btn-secondary v-tap" onClick={() => go('home')} style={{ marginTop: 10 }}>{t('evalResults.backToHome')}</button>
        </div>
      </div>
    );
  }
  ...
  const possible = result.total_possible ?? (state?.examMarks as number | undefined) ?? 80;
  ...
  const correctCount = sections.reduce(
    (acc: number, s: ResultSection) => acc + (s.questions || []).filter((q: ResultQuestion) => q.status === 'correct').length, 0
  );
    ...
    <div style={{ minHeight: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '72px 24px 32px' }}>
        <div className="v-enter" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32 }}>
          ...
          <div className="v-eyebrow" style={{ marginBottom: 8 }}>{t('evalResults.eyebrow')}</div>
          <h1 className="v-h1" style={{ fontSize: 30, textAlign: 'center' }}>{t('evalResults.title')}</h1>
        </div>

        <div className="v-card v-enter" style={{ padding: 32, marginBottom: 14, textAlign: 'center' }}>
          <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontWeight: 700, fontSize: 84, lineHeight: 1, color: 'var(--ink)' }}>{awarded}<span style={{ fontSize: 36, color: 'var(--muted-2)' }}>/{possible}</span></div>
          <div style={{ fontSize: 14, color: 'var(--muted)', marginTop: 14 }}>{t('evalResults.approxNote', { pct })}</div>
        </div>
        ...
        {correctCount > 0 && (
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            <div className="v-card-soft" style={{ flex: 1, textAlign: 'center', padding: '18px 12px' }}>
              <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 24 }}>{pct}%</div>
              <div className="v-eyebrow-sm" style={{ marginTop: 4 }}>{t('evalResults.overall')}</div>
            </div>
            <div className="v-card-soft" style={{ flex: 1, textAlign: 'center', padding: '18px 12px' }}>
              <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 24 }}>{correctCount}</div>
              <div className="v-eyebrow-sm" style={{ marginTop: 4 }}>{t('evalResults.questionsCorrect')}</div>
            </div>
          </div>
        )}
        ...
        {sections.length > 0 && (
          <>
            <VSectionHeader title={t('evalResults.bySection')} />
            <div className="v-card" style={{ padding: 0, marginBottom: 24, overflow: 'hidden' }}>
              {sections.map((s: ResultSection, i: number) => {
        ...
        {Array.isArray(result.improvement_areas) && result.improvement_areas.length > 0 && (
          <>
            ...
            <div className="v-card" style={{ padding: '14px 18px', marginBottom: 24, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {result.improvement_areas.map((a: string, i: number) => (
