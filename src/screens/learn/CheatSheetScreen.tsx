...
import type { ScreenProps, CheatSheet } from '../../types';

// ─────────────────────────────────────────────────────────────
//  Cheat sheet — one dense screen per chapter: the rules worth
//  memorising, the shortcuts, and the traps that cost marks.
//  Deliberately compact and high-contrast: this is meant to be
//  screenshotted the night before a test.
// ─────────────────────────────────────────────────────────────
...
export default function CheatSheetScreen({ go, state }: ScreenProps) {
  ...
  if (!sheet) {
    return (
      <div style={{ minHeight: '100%', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        <VTopBar transparent showBack onBack={() => go('chapter-topics')} title={t('sheet.topbar')} />
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 18, padding: 32, minHeight: '70vh' }}>
          ...
          <div style={{ fontFamily: 'Inter', fontSize: 14, color: 'var(--muted)', textAlign: 'center', lineHeight: 1.5 }}>
            {failed ? t('sheet.failed') : t('sheet.building', { chapter: chapter.title })}
          </div>
          {failed && <button className="v-btn-secondary v-tap" onClick={() => go('chapter-topics')}>{t('common:back')}</button>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '68px 20px 40px' }}>
        <div className="v-eyebrow" style={{ marginBottom: 5 }}>{t('sheet.eyebrow', { num: chapter.num })}</div>
        <h1 className="v-h1" style={{ fontSize: 26, marginBottom: 4, lineHeight: 1.15 }}>{sheet.title || chapter.title}</h1>
        <div style={{ fontFamily: 'Inter', fontSize: 11.5, color: 'var(--muted-2)', marginBottom: 18 }}>
          📸 {t('sheet.screenshotHint')}
        </div>
        ...
        <div className="v-card" style={{ padding: '18px 18px 4px', background: '#fff' }}>
          ...
          {!!sheet.tricks?.length && (
            <Block eyebrow={`⚡ ${t('sheet.tricks')}`}>
              {sheet.tricks.map((x, i) => (
