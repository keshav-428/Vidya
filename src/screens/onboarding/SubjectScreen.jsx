...
export default function SubjectScreen({ go, state, set }) {
  return (
    <OnbShell variant="warm" back={() => go('onb-class')}
      ...
      <h1 className="v-h1 v-enter" style={{ fontSize: 30, marginBottom: 8 }}>
        Pick a subject<br />to start with
      </h1>
      <p className="v-enter" style={{ fontFamily: 'Inter', fontSize: 14, color: 'var(--muted)', marginBottom: 28, lineHeight: 1.5 }}>
        You can add more subjects later from your profile.
      </p>
      <div className="v-enter">
        <TileGrid columns={2} value={state.subject} onSelect={(v) => set({ subject: v })}
          items={[
            { id: 'maths', label: 'Mathematics', sub: 'Numbers, algebra, geometry', glyph: 'π' },
