...
export default function LanguageScreen({ go, state, set }: ScreenProps) {
    ...
    <OnbShell variant="warm" back={() => go('splash')}
      ...
      <div className="v-enter">
        <TileGrid columns={2} value={state.language} onSelect={(v) => set({ language: v })}
          items={[
            { id: 'en', label: 'English', sub: t('language.englishSub'), glyph: 'Aa' },
            { id: 'hi', label: 'हिन्दी', sub: t('language.hindiSub'), glyph: 'अ' }
          ]} />
      </div>
    </OnbShell>
  );
