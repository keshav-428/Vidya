...
function TileGrid({ items, value, onSelect, columns = 2 }) {
  return (
...
function OnbShell({ variant = 'warm', back, progress, eyebrow, children, primaryDisabled, onPrimary, primaryLabel = 'Continue' }) {
  return (
...
export default function LanguageScreen({ go, state, set }) {
  return (
    <OnbShell variant="warm" back={() => go('role')}
      ...
      <h1 className="v-h1 v-enter" style={{ fontSize: 30, marginBottom: 8 }}>
        Which language are<br />you comfortable in?
      </h1>
      <p className="v-enter" style={{ fontFamily: 'Inter', fontSize: 14, color: 'var(--muted)', marginBottom: 28, lineHeight: 1.5 }}>
        Vidya will explain everything in this language.
      </p>
      <div className="v-enter">
        <TileGrid columns={2} value={state.language} onSelect={(v) => set({ language: v })}
          items={[
            { id: 'en', label: 'English', sub: 'For most CBSE/ICSE schools', glyph: 'Aa' },
            { id: 'hi', label: 'हिन्दी', sub: 'समझाएंगे आसान भाषा में', glyph: 'अ' }
          ]} />
      </div>
    </OnbShell>
  );
