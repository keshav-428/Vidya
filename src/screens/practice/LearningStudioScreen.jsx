...
function AskedPractice({ go, topic }) {
    ...
    return (
      <div style={{ minHeight: '100%', background: 'var(--bg)' }}>
        ...
        <div style={{ padding: '80px 24px 32px' }}>
          <div className="v-eyebrow" style={{ marginBottom: 8 }}>PRACTICE COMPLETE</div>
          <h1 className="v-h1" style={{ fontSize: 32, marginBottom: 8, lineHeight: 1.15 }}>
            {score === PRACTICE_QUESTIONS.length ? 'All correct.' : score >= 3 ? 'Nice work.' : 'Keep at it.'}
          </h1>
          <p className="v-body" style={{ marginBottom: 28 }}>
            You got <strong>{score} of {PRACTICE_QUESTIONS.length}</strong> on "{topic}".
          </p>
...
/* ─── Main LearningStudio ─── */
export default function LearningStudioScreen({ go, state }) {
  const [tab, setTab] = useState('learn');
  const asked = state?.askedTopic;
