...
export default function ConceptLibraryScreen({ go }: ScreenProps) {
  ...
  return (
    <div style={{ minHeight: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '72px 24px 32px' }}>
        <div className="v-eyebrow" style={{ marginBottom: 8 }}>{t('conceptLibrary.eyebrow')}</div>
        <h1 className="v-h1" style={{ fontSize: 30, marginBottom: 20 }}>{t('conceptLibrary.title')}</h1>

        <div style={{ position: 'relative', marginBottom: 24 }}>
          <input
            autoFocus
            placeholder={t('conceptLibrary.searchPlaceholder')}
            value={q}
        ...
        <div className="v-eyebrow" style={{ marginBottom: 14 }}>{q ? t('conceptLibrary.results') : t('conceptLibrary.recents')}</div>
          ...
          {filtered.map((a, i) => (
            <div key={a.t} className="v-tap" onClick={() => go('learning-studio')} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: "'Quicksand','Baloo 2','Nunito',system-ui,sans-serif", fontSize: 17 }}>{a.t}</div>
                <div style={{ fontSize: 11, color: 'var(--muted-2)', marginTop: 2 }}>{a.topic} · {a.tag}</div>
              </div>
              <VIcon name="chevron-right" size={16} color="var(--muted-2)" />
            </div>
          ))}
          {filtered.length === 0 && <div style={{ padding: 24, textAlign: 'center', color: 'var(--muted-2)', fontSize: 13 }}>{t('conceptLibrary.noMatches')}</div>}
        </div>
      </div>
    </div>
  );
