...
import type { Card, RealWorldUse, Scene, Visual, VideoItem } from '../types';

interface ThemeStyle { grad: string; accent: string; tagBg: string; tagColor: string; }
...
const VIDEO_GRADIENTS: Record<string, string> = {
...
// ── INLINE DIAGRAMS ─────────────────────────────────────────
function PizzaVisual({ slices = 4, filled = 1 }: { slices?: number; filled?: number }) {
...
function FractionBars({ numerator = 1, denominator = 4 }: { numerator?: number; denominator?: number }) {
  return (
...
function OneBar({ numerator, denominator, color }: { numerator: number; denominator: number; color: string }) {
  return (
...
function CompareBars({ a, b }: { a: { numerator: number; denominator: number }; b: { numerator: number; denominator: number } }) {
  return (
...
const VISUALS: Record<string, (p: Visual) => React.ReactElement> = {
  pizza: (p) => <PizzaVisual {...p} />,
  fractionBars: (p) => <FractionBars {...p} />,
  compareBars: (p) => <CompareBars a={p.a!} b={p.b!} />,
};

export function renderVisual(visual?: Visual): React.ReactElement | null {
  if (!visual || !VISUALS[visual.kind]) return null;
  return VISUALS[visual.kind](visual);
}

type CardRenderer = React.FC<{ card: Card; topic?: string }>;

// ── CARD: definition ────────────────────────────────────────
const DefinitionCard: CardRenderer = ({ card }) => {
...
// ── CARD: mistakes ──────────────────────────────────────────
const MistakesCard: CardRenderer = ({ card }) => {
  return (
...
// ── CARD: realLife ──────────────────────────────────────────
// Real-world uses come from the LLM (/real-world), mapped onto the scene
// layout. ConceptScreen preloads these up front (see mapScenes); this card
// only fetches on its own as a fallback when nothing was preloaded.
const SCENE_THEMES = ['saffron', 'indigo', 'green'];

/** Map raw /real-world uses onto the (max 3) themed scenes the card renders. */
export function mapScenes(uses: RealWorldUse[]): Scene[] {
...
const RealLifeCard: CardRenderer = ({ card, topic }) => {
  const preloaded = !!(card.scenes && card.scenes.length);
  const [scenes, setScenes] = useState<Scene[]>(card.scenes || []);

  useEffect(() => {
    if (preloaded) return;   // ConceptScreen already loaded these up front
    let alive = true;
    api.realWorld(topic || card.heading || 'mathematics', 6)
      .then((uses) => {
        const mapped = mapScenes(uses);
        if (alive && mapped.length) setScenes(mapped);
      })
      .catch(() => {});
    return () => { alive = false; };
  }, [topic, card.heading, preloaded]);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {scenes.map((s, i) => {
        const th = THEMES[s.theme || 'indigo'] || THEMES.indigo;
        return (
          <div key={i} style={{ background: th.grad, borderRadius: 16, padding: '14px 16px', border: `1px solid ${th.accent}22` }}>
            <div style={{ fontFamily: 'Inter', fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', color: th.accent, marginBottom: 5, opacity: 0.8 }}>
              {s.label}
            </div>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', background: th.tagBg, borderRadius: 999, padding: '4px 10px', fontFamily: 'Inter', fontSize: 11, fontWeight: 700, color: th.tagColor, border: `1px solid ${th.accent}33` }}>
              {s.tag}
            </div>
          </div>
        );
...
// ── CARD: examples ──────────────────────────────────────────
const ExamplesCard: CardRenderer = ({ card }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
...
// Pull the 11-char YouTube id out of the common URL shapes (watch, youtu.be,
// embed, shorts). Returns null for anything we can't embed.
...
/** Keep only embeddable YouTube results, trimmed to the fields the card uses. */
export function mapVideos(items: VideoItem[]): VideoItem[] {
...
// ── CARD: videos ────────────────────────────────────────────
// YouTube results come from /search-videos. ConceptScreen preloads them up
// front (see mapVideos); this card only fetches on its own as a fallback.
const VideosCard: CardRenderer = ({ card, topic }) => {
  const preloaded = !!(card.videos && card.videos.length);
  const [videos, setVideos] = useState<VideoItem[] | null>(preloaded ? card.videos! : null);   // null = loading
  const [playingIdx, setPlayingIdx] = useState<number | null>(null);   // which video is playing inline
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {videos.map((v, i) => {
        const grad = VIDEO_GRADIENTS[v.theme || 'purple'] || VIDEO_GRADIENTS.purple;
        const id = youtubeId(v.url);
        const playing = playingIdx === i && !!id;
        return (
          <div key={i} style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
            {/* 16:9 media area — embedded player once tapped, thumbnail before that */}
            <div style={{ position: 'relative', width: '100%', paddingTop: '56.25%', background: playing ? '#000' : (v.thumbnail ? `center/cover no-repeat url(${v.thumbnail})` : grad) }}>
              {playing ? (
                <iframe
                ...
                <div
                  className="v-tap"
                  onClick={() => { if (id) setPlayingIdx(i); else if (v.url) window.open(v.url, '_blank', 'noopener'); }}
...
export function renderCard(card: Card, topic?: string): React.ReactElement {
