...
import DiagnosticQuizScreen     from './screens/quiz/DiagnosticQuizScreen';
import DiagResultsScreen        from './screens/quiz/DiagResultsScreen';
...
export const SCREEN_ROUTES = {
  ...
  'diagnostic-results':   '/quiz/diagnostic-results',
...
const DEFAULT_STATE = { name: 'Arjun', conceptLayout: 'cards' };

// localStorage key for a given user — guests keep the legacy key, signed-in
// users get their own per-uid bucket (so signup = fresh, login = restore).
const storageKeyFor = (uid) => (uid ? `vidya_state_${uid}` : 'vidya_state');
...
function AppInner() {
  ...
  const [state, setState] = useState(() => loadState(storageKeyFor(null)));
  ...
  const syncedProfile = useRef(null);
  useEffect(() => {
  ...
  const currentScreen = PATH_TO_SCREEN[location.pathname] || 'splash';
  ...
  const ctx = { state, set, go, currentScreen };
    ...
    <AppContext.Provider value={ctx}>
      <div style={{ width: '100%', height: '100%', overflow: 'auto', position: 'relative' }}>
        <Routes>
          ...
          <Route path="/quiz/diagnostic"        element={<DiagnosticQuizScreen     go={go} state={state} set={set} />} />
          <Route path="/quiz/diagnostic-results" element={<DiagResultsScreen       go={go} state={state} set={set} />} />
