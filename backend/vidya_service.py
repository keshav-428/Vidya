...
SYSTEM_PROMPT = (
    """
You are Vidya, a friendly and patient study teacher for Class {grade} students.
...
def generate_daily_greeting(student_id: str, name: str, grade: int, language: str = "English"):
    ...
    TASK: Generate a warm, simple, personalized 2-line greeting for the student opening the app today.
    ...
    try:
        ...
        print(f"Error generating daily greeting: {e}")
        return f"Hey {name.split()[0]}! Ready to practice today? Pick a topic and let's go!"

def generate_quiz_feedback(student_id: str, topic: str, score: int, total: int, mistakes: list, language: str = "English"):
    """Analyzes quiz performance and provides 3-part feedback."""
    memory = get_student_memory(student_id)
...
def explain_mistake(question: str, user_answer: str, correct_answer: str, grade: int = 6, language: str = "English"):
    ...
    Be very encouraging and gentle. Never be harsh.
    """
...
- Specific and relatable to a {age}-year-old in India (for example: cricket, pocket money, shopping in the market, bus fares, mobile recharge, sharing food)
- NOT generic textbook examples like "used in cooking" or "used in engineering"
- A short, clear title (3-5 words) and a 1-sentence explanation showing the exact connection
